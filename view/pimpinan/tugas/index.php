...
$noverif = $con->query("SELECT COUNT(*) AS total FROM tugas WHERE verif = 1 ")->fetch_array();
?>
        ...
        <div class="col-lg-12 col-md-12 col-12 my-3">
            ...
            <div class="d-flex justify-content-between align-items-center">
                <div class="mb-2 mb-lg-0">
                    <h4 class="mb-0"><i class="bi bi-calendar-range-fill me-2"></i>Data Perintah Tugas</h4>
                </div>
            </div>
        </div>
        ...
        <div class="col-12">
            <div class="card card-body border border-dark-danger">
                <a class="btn btn-primary" data-bs-toggle="collapse" href="#collapseExample" role="button" aria-expanded="false" aria-controls="collapseExample">
                    Data Perintah Tugas Belum di Verifikasi <span class="fw-bold badge bg-danger"><?= $noverif['total'] ?> Data</span>
                </a>
                    ...
                    <div class="table-responsive mt-2">
                        <table id="tbl" class="table table-bordered table-striped dataTable">
                            ...
                            <tbody>
                                <?php
                                $no = 1;
                                $data = $con->query("SELECT * FROM tugas WHERE verif = 1 ORDER BY id_tugas ASC");
                                ...
                                ?>
                                    <tr>
                                        ...
                                        <td align="center" width="13%">
                                            <div class="d-grid">
                                                <a href="verif?id=<?= $row['id_tugas'] ?>&v=2" class="btn bg-success text-white btn-xs alert-setuju" title="Setujui"><i class="fas fa-check-circle me-1"></i>Setujui</a>
                                                <span data-bs-target="#tolak<?= $row['id_tugas']; ?>" data-bs-toggle="modal" class="btn bg-danger text-white btn-xs mt-1" title="Tolak"><i class="fa fa-times-circle me-1"></i> Tolak</>
                                            </div>
                ...
                <div class="table-responsive">
                    <table id="example1" class="table table-bordered table-hover table-striped dataTable">
                        ...
                        <tbody>
                            <?php
                            $no = 1;
                            $data = $con->query("SELECT * FROM tugas WHERE verif != 1 ORDER BY id_tugas DESC");
                            while ($row = $data->fetch_array()) {
                                $tgl1 = $row['tgl_mulai'];
                                $tgl2 = date('Y-m-d', strtotime('-1 days', strtotime($tgl1)));
                                $a = date_create($tgl2);
...
if (isset($_POST['submit_tolak'])) {
    ...
    if ($update) {
        $_SESSION['pesan'] = "Data Perintah Tugas di Tolak";
